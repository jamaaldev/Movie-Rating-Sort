...
let dataMovies = 'http://localhost:3000/api/post'
window.onload = () => {
    GetTheMovies();
...
function SortByRate(DomMovies) {
    ...
    DomMovies.filter((Movie) => {
        ...
        let miniRate = Movie.parentElement.parentElement.children[0].children[0].children[0];
        getMyRate(rat)
...
function ShowMovies(movies) {
    ...
    for (movie of movies) {
        ...
        const moviePost = `./postmovie.html?id=${movie.id}`;
        html = ` 
        <a class="movieLink" href="${moviePost}"> 
